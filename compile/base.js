...
var _typeof = "function" == typeof Symbol && "symbol" == _typeof2(Symbol.iterator) ? function (t) {
  ...
  }, _bindFooterEvents: function _bindFooterEvents() {
  ...
  }, _bindProgressBarEvents: function _bindProgressBarEvents() {
    function t(t) {
      this.progress = t, this.prg = 0, this.width = document.documentElement.clientWidth, this.timer = 0;
        ...
        if ("object" !== (void 0 === t ? "undefined" : _typeof(t))) return Math.random() * t;var r = t[1] - t[0];return Math.random() * r * 10 + t[0];
      } }, new t(document.getElementById("progress")).start();
  }, _bindInstallEvents: function _bindInstallEvents() {
    $("#btn-submit").on("click", function (t) {
      t.preventDefault();var r = $("#dbname").val(),
          o = $("#dbip").val(),
          e = $("#username").val(),
          n = $("#pwd").val();return "string" != typeof r || 0 === r.length ? ($("#dbname").get(0).focus(), $("#btn-submit").text("立即安装"), alert("数据库名不能为空")) : "string" != typeof o || 0 === o.length ? ($("#dbip").get(0).focus(), $("#btn-submit").text("立即安装"), alert("数据库IP地址不能为空")) : "string" != typeof e || 0 === e.length ? ($("#username").get(0).focus(), $("#btn-submit").text("立即安装"), alert("数据库用户名不能为空")) : "string" != typeof n || 0 === n.length ? ($("#pwd").get(0).focus(), $("#btn-submit").text("立即安装"), alert("数据库用户密码不能为空")) : ($("#btn-submit").text("正在安装……"), void $.ajax({ url: "/install", type: "POST", data: { dbname: r, dbip: o, username: e, password: n }, dataType: "json", success: function success(t) {
          if (-1 === t.status) {
            ...
            r--, $("#btn-submit").text("安装成功，正准备跳转首页……" + r + "s"), 0 === r && (clearInterval(o), window.location.href = "/");
          }, 1e3);
        } }));
    });
  }, _bindSearchContentsEvents: function _bindSearchContentsEvents() {
